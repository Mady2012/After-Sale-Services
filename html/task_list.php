...
if ($role === 'technician') {
    if (!empty($projectid)) {
        $sql = "SELECT TaskID, ProjectID, TaskName, TechnicianID, Description, Status  
                FROM task 
                WHERE TechnicianID = :techid AND ProjectID = :projectid
                ORDER BY TaskID DESC";
        $stmt = $conn->prepare($sql);
        $stmt->execute(['techid' => $tech_id, 'projectid' => $projectid]);
    } else {
        $sql = "SELECT TaskID, ProjectID, TaskName, TechnicianID, Description, Status  
                FROM task 
